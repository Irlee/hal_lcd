...
uint16_t LCD_ReadPoint(uint16_t x,uint16_t y)
{
	...
	LCD_SetCursor(x,y);	    
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x7789)LCD_WR_REG(0X2E);//9341/6804/3510/1963/7789 发送读GRAM指令
	...
	opt_delay(2);	  
 	...
 	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0x7789)		//9341/NT35310/NT35510要分2次读出
 	{
		opt_delay(2);	  
		b=LCD_RD_DATA(); 
	...
	else if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0x7789)return (((r>>11)<<11)|((g>>10)<<5)|(b>>11));//ILI9341/NT35310/NT35510需要公式转换一下
	else return LCD_BGR2RGB(r);						//其他IC
}			 
//LCD开启显示
void LCD_DisplayOn(void)
{					   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x7789)LCD_WR_REG(0X29);	//开启显示
...
{	   
	if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X1963||lcddev.id==0x7789)LCD_WR_REG(0X28);	//关闭显示
...
void LCD_Scan_Dir(uint8_t dir)
{
	...
	if(lcddev.id==0x9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963||lcddev.id==0x7789)//9341/6804/5310/5510/1963/7789,特殊处理
	{
		switch(dir)
		{
...
void LCD_Display_Dir(uint8_t dir)
{
	if(dir==0)			//竖屏
	{
		...
		lcddev.width=240;
		lcddev.height=320;
		if(lcddev.id==0X9341||lcddev.id==0X6804||lcddev.id==0X5310||lcddev.id==0x7789)
		{
			lcddev.wramcmd=0X2C;
	 		lcddev.setxcmd=0X2A;
			lcddev.setycmd=0X2B;  	 
...
{    
	...
	twidth=sx+width-1;
	theight=sy+height-1;
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X6804||lcddev.id==0x7789||(lcddev.dir==1&&lcddev.id==0X1963))
	{
		LCD_WR_REG(lcddev.setxcmd); 
		LCD_WR_DATA(sx>>8); 
		LCD_WR_DATA(sx&0XFF);	 
...
void LCD_keyboard_display(void)
{
  LCD_Clear(WHITE);
  //POINT_COLOR=CYAN;			//设置字体为black 
	POINT_COLOR = DARKBLUE;
  //LCD_Fill(1,120, 240, 320, CYAN);
	LCD_Fill(1*wide,320-5*high, 1*wide+12*8-1, 320-5*high+23, CYAN);
  LCD_DrawLine(0, 320-4*high, 240, 320-4*high);		//row 1
  LCD_DrawLine(0, 320-3*high, 240, 320-3*high);		//row 2
  LCD_DrawLine(0, 320-2*high, 240, 320-2*high);		//row 3
  LCD_DrawLine(0, 320-1*high, 180, 320-1*high);		//row 4
  LCD_DrawLine(1*wide, 320-4*high, 1*wide, 320);		//column 1
  LCD_DrawLine(2*wide, 320-4*high, 2*wide, 320-1*high);		//column 2
  LCD_DrawLine(3*wide, 320-4*high, 3*wide, 320);		//column 3
  LCD_ShowNum(0.5*wide-12, 320-0.5*high-12, 0, 1, 24);
  LCD_ShowNum(0.5*wide-12, 320-1.5*high-12, 1, 1, 24);
  LCD_ShowNum(0.5*wide-12, 320-2.5*high-12, 4, 1, 24);
  LCD_ShowNum(0.5*wide-12, 320-3.5*high-12, 7, 1, 24);
  LCD_ShowNum(1.5*wide-12, 320-1.5*high-12, 2, 1, 24);
  LCD_ShowNum(1.5*wide-12, 320-2.5*high-12, 5, 1, 24);
  LCD_ShowNum(1.5*wide-12, 320-3.5*high-12, 8, 1, 24);
  LCD_ShowNum(2.5*wide-12, 320-1.5*high-12, 3, 1, 24);
  LCD_ShowNum(2.5*wide-12, 320-2.5*high-12, 6, 1, 24);
  LCD_ShowNum(2.5*wide-12, 320-3.5*high-12, 9, 1, 24);
  LCD_ShowString(1*wide+25, 320-0.5*high-12, 200 , 24, 24, (uint8_t*)"Enter");
  LCD_ShowString(3*wide+5, 320-1*high-12, 200 , 24, 24, (uint8_t*)"Back");
  LCD_ShowString(3*wide+10, 320-2.5*high-12, 200 , 24, 24, (uint8_t*)"<--");
  LCD_ShowString(3*wide+5, 320-3.5*high-12, 200 , 24, 24, (uint8_t*)"Menu");
  POINT_COLOR = BLACK;
}
